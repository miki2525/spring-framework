...
import pl.training.shop.payments.PaymentFixtures;
...
class PaymentRestAdapterIntegrationTest {
    ...
    void given_payment_when_get_by_id_then_returns_payment() throws Exception {
        entityManager.persist(createEntity("STARTED"));
        entityManager.flush();
    ...
    @Test
    void when_get_by_invalid_id_then_returns_bad_request() throws Exception {
        mockMvc.perform(get("/api/payments/1" )
                        .accept(APPLICATION_JSON)
                        .locale(new Locale("pl", "PL")))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.description", is("Niepoprawne dane wejściowe")));
    }
